...
/**
 * {@link Operation} decorator that allows yielding the database to other transactions.
 *
 * @author Marten Gajda
 */
public final class Yieldable<T> implements Operation<T>
{
    ...
    public Yieldable(@NonNull Operation<T> delegate)
    {
        mDelegate = delegate;
    }
    ...
    {
        return mDelegate.contentOperationBuilder(transactionContext).withYieldAllowed(true);
    }
}
